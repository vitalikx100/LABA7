...
public class Functions {
    ...
    public static double Integral(Function function, double left, double right, double SamplingStep) {
        int step = (int) Math.ceil((right - left) / SamplingStep);
        double total = 0;
        double x = left;
        for (int i = 0; i < step - 1; ++i, x += SamplingStep) {
            double m = (function.getFunctionValue(x) + function.getFunctionValue(x + SamplingStep)) / 2;
            total += SamplingStep * m;
        }
